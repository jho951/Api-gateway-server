...
- 이 레포의 계약 동기화 기준 파일은 루트 [CONTRACT_SYNC.md](CONTRACT_SYNC.md) 입니다.
- 계약 영향 변경이 있으면 `CONTRACT_SYNC.md`를 먼저 갱신하고, 그 다음 구현을 맞춥니다.
...
- Gateway는 인증 성공 후 내부 서비스에 `X-User-Id`를 주입합니다.
- 외부에서 들어온 `X-User-Id`는 신뢰하지 않습니다.
