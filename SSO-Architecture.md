...
flowchart TB
    subgraph GatewayLayer[Gateway]
        G1[Route by path]
        G2[Header format check]
        G3[Optional shallow exp check]
        G4[Forward Authorization header]
        G5[Common error / logging / rate limit]
    end
...
sequenceDiagram
    ...
    G->>G: Header format and obvious-malformed-token pre-check
    opt exp pre-check enabled
        G->>G: Read payload exp without trusting signature
    end
...
| 서비스 | 역할 | 책임 | 비책임 |
| --- | --- | --- | --- |
| `Api-gateway-server` | 외부 요청의 단일 진입점 | 경로 기반 라우팅, 헤더 형식 체크, 명백한 비정상 JWT 차단, 선택적 얕은 `exp` 체크, `Authorization` 헤더 전달, 공통 예외 처리, 로깅, rate limit | 토큰 서명 신뢰 판단, 사용자 상태 확인, 권한 판단, 세션/철회 상태 반영, JWT 발급 |
...
- `User-service`는 "사용자 도메인의 기준 정보를 소유하고 관리하는 계층"이다.
